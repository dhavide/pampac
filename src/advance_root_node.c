...
advance_root_node (PTnode ** root, options_struct * opts) {
  ...
  /* Replace root node iff it has a solitary GREEN child. */
  has_solitary_child = count_children (root_tmp) == 1;
  has_converged = root_tmp->color == GREEN;
  while ( has_solitary_child && has_converged ) {
    ...
    for (k = 0; k < root_tmp->max_children; k++)
      if (root_tmp->child[k]!=NULL)
        break;
    if (root_tmp->child[k]->color != GREEN)
      break;
    ...
    *root = root_tmp;
    ...
    /* Check whether new root node has a solitary GREEN child. */
    has_solitary_child = count_children (root_tmp) == 1;
    has_converged = root_tmp->color == GREEN;
  }
